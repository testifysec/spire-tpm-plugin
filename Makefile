...
build: $(BUILD_TARGETS)
$(BUILD_TARGETS):
	CGO_ENABLED=1 GOOS=$(target_os) GOARCH=$(target_architecture) go build -ldflags="-s -w" -o $(BUILD_DIR)/$(target_os)/$(target_architecture)/$(target_binary) cmd/$(target_binary)/main.go
...
docker-build:
	CGO_ENABLED=1 GOOS=$(TARGETOS) GOARCH=$(TARGETARCH) go build -ldflags="-s -w" -o ${BINARY} cmd/${BINARY}/main.go
